...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ onToggleChat, onTogglePlayers, onToggleControls, activeView, chatOpen }) => {
  ...
  const iconColor = "text-[var(--color-bg-cream-light)]"; 
  const activeIconColor = "text-[var(--color-accent-gold)]";
  
  const activeTextClass = "text-[var(--color-accent-gold)] font-semibold";
  const inactiveTextClass = `${iconColor} opacity-80 group-hover:opacity-100 group-hover:text-[var(--color-accent-gold)]`;
  ...
  return (
    ...
    >
      ...
      <button onClick={onToggleChat} className={`${baseClass} group ${chatOpen ? activeTextClass : inactiveTextClass}`}>
        <MessageSquare size={22} className={`${chatOpen ? activeIconColor : iconColor} group-hover:text-[var(--color-accent-gold)] transition-colors`} />
      ...
      <button onClick={onToggleControls} className={`${baseClass} group ${activeView === 'controls' && !chatOpen ? activeTextClass : inactiveTextClass}`}>
        <Settings size={22} className={`${activeView === 'controls' && !chatOpen ? activeIconColor : iconColor} group-hover:text-[var(--color-accent-gold)] transition-colors`} />
