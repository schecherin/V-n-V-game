...
const ReflectionPhase = ({ 
  ...
  const hasActivatableRole = player?.current_role_name !== undefined && 
  ...
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/execute-role-action`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 p-8">
      ...
      {isCurrentUserHost && (confirmed || phaseExpired) && (
        <div className="mt-6 flex flex-col items-center gap-2">
          <p className="text-sm text-gray-600">Als host kun je de minigame starten:</p>
          <Button 
