...
const PlayerSlotHorizontal: React.FC<PlayerSlotHorizontalProps> = ({ player, isCurrentUser }) => {
  ...
  const avatarBgColor = player.id % 2 === 0 ? 'bg-gold' : 'bg-cream';
  const avatarTextColor = 'text-brown-dark';

  return (
    <div className={`flex-shrink-0 w-24 flex flex-col items-center p-2 mx-1.5 bg-cream-light rounded-lg border-2 ${currentBorderColor} shadow-md`}>
      <div className="relative mb-1.5">
        ...
        {!player.isHost && player.isReady && <CheckCircle className="absolute -top-1 -right-1 h-5 w-5 text-accent-green bg-cream-light rounded-full p-0.5 shadow" />}
        {!player.isHost && !player.isReady && <Clock className="absolute -top-1 -right-1 h-5 w-5 text-accent-gold bg-cream-light rounded-full p-0.5 shadow" />}
      </div>
      <span className="text-xs font-medium text-brown-dark truncate w-full text-center">{player.username}</span>
