...
const BottomNavBar: React.FC<BottomNavBarProps> = ({
  ...
  const baseClass =
    "flex flex-col items-center justify-center py-2 px-1 flex-1 transition-colors duration-150 focus:outline-none focus-visible:bg-[var(--color-border-gold)]/20";
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-30 flex items-center justify-between px-4 py-2 bg-brown-dark border-t-2 border-gold/50 shadow-lg">
      <button
        onClick={onTogglePlayers}
        className={`${baseClass} group ${
          activeView === "players" && !chatOpen
            ? activeTextClass
            : inactiveTextClass
        }`}
      >
        <Users
          size={22}
          className={`${
            activeView === "players" && !chatOpen ? activeIconColor : iconColor
