    ...
    >
      ...
      <div className="flex-grow p-3 space-y-2.5 overflow-y-auto bg-cream">
        {chatMessages.map((msg, index) => (
          <div key={index} className={`text-sm py-1.5 px-2.5 rounded-lg shadow-sm max-w-[85%] break-words ${msg.user === "You" ? 'bg-accent-gold/30 ml-auto text-right' : (msg.user === "System" ? 'bg-brown-medium/20 text-center w-full max-w-full' : 'bg-cream-light border border-gold/50 mr-auto')}`}>
            {msg.user !== "System" && <span className={`font-semibold text-xs ${msg.user === "You" ? 'text-accent-gold' : 'text-brown-medium'}`}>{msg.user}: </span>}
      ...
      <div className="p-2 border-t border-gold/50 bg-brown-dark grid grid-cols-6 gap-1">
        {emotes.map(emote => (
          <Button key={emote.name} variant="ghost" size="sm" onClick={() => handleEmoteSend(emote.char)} className="text-2xl p-1 aspect-square text-cream-light hover:bg-gold/40">
            {emote.char}
          </Button>
        ))}
      </div>
