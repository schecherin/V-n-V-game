...
// It's better to have a single Supabase client instance, but for this self-contained example:
 ...
 */
export const assignRolesToPlayers = async (gameId: string, players: Player[]): Promise<{ success: boolean; error?: any }> => {
    ...
    try {
        ...
        const sortedRoles = (allRoles as Role[]).sort((a, b) => tierOrder[a.tier] - tierOrder[b.tier]);

        // Step 3: Separate unique roles from the non-unique "Worshipper" roles.
        const uniqueRoles = sortedRoles.filter(r => r.role_name !== 'Vice worshipper' && r.role_name !== 'Virtue seeker');
        const worshipperRoles = sortedRoles.filter(r => r.role_name === 'Vice worshipper' || r.role_name === 'Virtue seeker');

        if (worshipperRoles.length === 0) {
            throw new Error("Game setup error: 'Worshipper' or 'Seeker' roles are missing from the database.");
        }

        // Step 4: Shuffle players to ensure random role assignment.
        const shuffledPlayers = shuffleArray(players);
        const assignments: { player_id: string; current_role_name: string; original_role_name: string }[] = [];
        ...
        for (const player of shuffledPlayers) {
            ...
            if (availableUniqueRoles.length > 0) {
                ...
                assignedRoleName = worshipperRoles[assignments.length % worshipperRoles.length].role_name;
            }
            assignments.push({
